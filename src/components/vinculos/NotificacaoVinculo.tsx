...
export function NotificacaoVinculo({ vinculo, onClose, onRespondido }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
      <div className="bg-brand-dark-2 w-full max-w-md md:max-w-3xl rounded-2xl border border-brand-dark-5 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Coluna da Esquerda (Informações e Limitações) */}
            <div className="space-y-5">
              {/* Informações da solicitação */}
              <div className="text-center md:text-left space-y-2">
                <p className="text-xs text-gray-500">O despachante profissional</p>
                <p className="text-lg font-black text-white">{vinculo.despachante_nome}</p>
                <p className="text-xs text-gray-400 leading-normal">solicitou acesso ao acervo digital da sua conta.</p>
              </div>

              {/* Mensagem enviada pelo despachante */}
              {vinculo.mensagem_solicitacao && (
                <div className="bg-brand-dark-3 rounded-xl p-3.5 border border-brand-dark-5 text-xs text-gray-400 leading-relaxed italic">
                  "{vinculo.mensagem_solicitacao}"
                </div>
              )}

              {/* Declaração LGPD e Limitações */}
              <div className="space-y-3 text-[11px] text-gray-500 leading-relaxed pt-2 border-t border-brand-dark-5/40">
                <div className="flex gap-2 items-start">
                  <Info size={14} className="text-brand-blue shrink-0 mt-0.5" />
                  {vinculo.permite_edicao ? (
                    <p>
                      <strong>Permissão de Edição:</strong> Com sua autorização e aceite do termo, este despachante poderá cadastrar, alterar e excluir armas, guias e manejos diretamente na sua conta.
                    </p>
                  ) : (
                    <p>
            ...
            {/* Coluna da Direita (Compartilhamento e Ações) */}
            <div className="space-y-5 flex flex-col justify-between">
              <div className="space-y-4">
                ...
                {vinculo.permite_edicao && (
                  <div className="bg-brand-dark-3 border border-brand-dark-5 rounded-xl p-4 space-y-3">
                    ...
                    <label className="flex items-start gap-2.5 cursor-pointer group pt-2 border-t border-brand-dark-5/50">
                      ...
                      />
                      <span className="text-[11px] text-gray-300 group-hover:text-white leading-normal transition-colors font-semibold font-semibold">
                        Estou ciente e autorizo este despachante a gerenciar, atualizar e editar os dados do meu acervo.
                      </span>
                    </label>
                  </div>
